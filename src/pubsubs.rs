...
use crate::{redis, players, bcrypt_cache};
...
async fn change_password_handler(md5: &str) { // XX: do i even need this? it's not bound per user.
...
async fn change_username_handler(raw: &str) {
    ...
    let _user = players.get_id(data["userID"].as_i64().unwrap() as i32).await.unwrap();
    let user = _user.read().await;

    let mut packet_bytes = handlers::notification(
        &format!("Your username has been changed to {}!", data["newUsername"].as_str().unwrap())
    );
...
async fn notification_handler(raw: &str) {
    ...
    user.enqueue(
        handlers::notification(data["message"].as_str().unwrap())
    ).await;
}
